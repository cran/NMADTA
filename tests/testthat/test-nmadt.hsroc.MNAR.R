...
test_that("nmadt.hsroc.MNAR is working and the results are reproducible", {
  ...
  set.seed(9)
  kang.out.hsroc.MNAR <- nmadt.hsroc.MNAR(nstu=12, K=2, data=kangdata, testname=c("D-dimer","Ultrasonography"),
  ...
  expect_equal(as.numeric(unlist(kang.out.hsroc.MNAR$Se)),as.numeric(unlist(Se)) + 0.06)
})
