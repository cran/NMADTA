...
#' }
#' @return No return value, called for side effects (generating and saving plots).
#' @export
hierarchical.plot=function(nstu, K, data, testname, output, directory,
  ...
  if(is.element("density", typeofplot)){
    ...
    density_hierarchical(samp.gen = output$rawOutput, testname = testname, K = K, 
                         n.chains = n.chains, dirc = directory)
  }
  if(is.element("forest", typeofplot)){
    message("Start saving forest plots...\n")
    forestplot.hierarchical(K = K, nstu = nstu, dat = data, testname = testname, 
                            samp = output$rawOutput, dirc = directory)
  }
  if(is.element("sroc", typeofplot)){
    message("Start saving SROC plots...\n")
    sroc.hierarchical(K = K, nstu = nstu, dat = data, testname = testname, 
                      samp = output$rawOutput, dirc = directory)
  }
  if(is.element("contour", typeofplot)){
    message("Start saving contour plots...\n")
    contour_hierarchical(samp.gen = output$rawOutput, K = K, testname = testname, dirc = directory)
  }
  #options(warn = 0)
}
