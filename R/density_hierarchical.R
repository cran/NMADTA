...
density_hierarchical <- function(x,
  ...
  if (tolower(x$model) != "hierarchical")
    stop("density_hierarchical() is for hierarchical models only.")
  ...
  for (i in seq_len(K)) {
    ...
    dens.Se[i, ] <- c(max(tempdens.Se$y),
                      quantile(tempSe, 0.001),
                      quantile(tempSe, 0.999))
    tempdens.Sp <- density(tempSp)
    dens.Sp[i, ] <- c(max(tempdens.Sp$y),
                      quantile(tempSp, 0.001),
                      quantile(tempSp, 0.999))
  }
